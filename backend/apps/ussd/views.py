...
def _get_nalo_token():
    """Get a fresh Nalo API token (no caching to avoid expiration)."""
    ...
    try:
        ...
        data = resp.json()
        logger.info(f'Nalo token response: success={data.get("success")}')
        ...
        if token:
            logger.info(f'Fresh Nalo token obtained')
            return token
        
        logger.error(f'Nalo: could not extract token from response')
        return None
...
def _generate_trans_hash(merchant_id, account_number, amount, reference):
    ...
    logger.info(f'Generated trans_hash for {reference}: {trans_hash[:32]}...')
    return trans_hash
...
def _trigger_momo_payment(msisdn, amount, reference, description, account_name, network="MTN"):
    ...
    try:
        ...
        if clean_phone.startswith('233'):
            account_number = '0' + clean_phone[3:]
...
class CheckPaymentStatusView(View):
    ...
    def post(self, request, reference):
        ...
        if result.get('success'):
            ...
            cand_name = payment.candidate_name if hasattr(payment, 'candidate_name') else 'your candidate'
            _send_sms_confirmation(payment.phone, cand_name, payment.votes_bought or 1, reference)
